...
public class ConvertShortSchedule {
	...
	private int hyperTransmissions;
	
	private int repetitiveTransmissions;
	...
	public void convert(String inputFile) throws IOException {
		...
		schedulable = -1;
		System.out.print("top\tid\tflows\tshortSchedule\tau\tch\tpiggyback");
		System.out.print("\tSch\toaPer\toaMax\ttime\tmaxBuf\tprod\thyper\thyp-trans\trep-trans\tnonempty-slots");
		System.out.println();
	...
	private void fillSchedulingContents() throws IOException {
		...
		tokenizer.nextToken();
		this.hyperTransmissions = Integer.parseInt(tokenizer.nextToken());
		tokenizer.nextToken();
		this.repetitiveTransmissions = Integer.parseInt(tokenizer.nextToken());
		tokenizer.nextToken();
		this.nonEmptySlots = Integer.parseInt(tokenizer.nextToken());
	}
	...
	private void resetSchedulingContents() {
		...
		this.hyperPeriod = 0;
		this.hyperTransmissions = 0;
		this.repetitiveTransmissions = 0;
	...
	private void processLine(String line) throws IOException {
		...
		} else if (line.startsWith("shortSchedule")) {
			this.shortSchedule = line.contains("true") ? 1: 0;
		}
	}


	private void outputRow() {
		System.out.printf("%d\t%d\t%d\t%d\t%.3f\t%d\t%d", this.topologyId, this.id, 
				this.flows, this.shortSchedule, this.actualUtilization, this.channels, this.piggyback);
		...
		System.out.print("\t"+this.hyperTransmissions);
		System.out.print("\t"+this.repetitiveTransmissions);
		System.out.print("\t"+this.nonEmptySlots);
		System.out.println();
	}

	public static void main(String[] args) throws IOException {
		ConvertShortSchedule conv = new ConvertShortSchedule();
		conv.convert(args[0]);
	}
}
