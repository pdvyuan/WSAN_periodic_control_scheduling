...
public class ConvertEffectOfImportantMetrics {
	...
	public void convert(String inputFile) throws IOException {
		...
		String line;
		System.out.print("top\tnGWs\tnPaths\trouting\tflows\troutableFlows\treliability\thops\ttime\tmetric\t");
	...
	private void processLine(String line) {
		if (line.startsWith("GWs=")) {
			...
			StringTokenizer tokenizer = new StringTokenizer(line);
			tokenizer.nextToken();
			this.metric = Integer.parseInt(tokenizer.nextToken());
		}
	}
	...
	private void outputRow() {
		if (this.flows != 0) {
			System.out.printf("%d\t%d\t%d\t%d\t%d\t%d\t%.3f\t%.3f\t%d\t%d\t", 
					...
					this.routableFlows, this.meanReliability, this.meanHops, this.time, this.metric);
	...
	public static void main(String[] args) throws IOException {
		ConvertEffectOfImportantMetrics conv = new ConvertEffectOfImportantMetrics();
		conv.convert(args[0]);
	}
}
