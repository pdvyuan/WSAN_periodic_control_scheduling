...
public class ConvertRoutingComparison {
	...
	public void convert(String inputFile) throws IOException {
		...
		String line;
		System.out.print("top\tnGWs\tnPaths\trouting\tflows\troutableFlows\treliability\thops\ttime\t");
	...
	private void outputRow() {
		if (this.flows != 0) {
			System.out.printf("%d\t%d\t%d\t%d\t%d\t%d\t%.3f\t%.3f\t%d\t", 
					this.topologyId, this.numGWs, this.numPaths, this.routingType, this.flows, 
					this.routableFlows, this.meanReliability, this.meanHops, this.time);
	...
	public static void main(String[] args) throws IOException {
		ConvertRoutingComparison conv = new ConvertRoutingComparison();
		conv.convert(args[0]);
	}
}
